...
void	kdo_allocImage(Kdo_Vulkan *vk, Kdo_VkImage *image, VkMemoryPropertyFlags memoryFlags, void (*info)(VkExtent3D, VkImageCreateInfo*))
{
	...
	{
		(*info)(image->div[i].extent, &imageInfo);
	...
	allocInfo.memoryTypeIndex   = kdo_findMemoryType(vk, image->memoryFilter, memoryFlags);
...
Kdo_VkImage	kdo_mergeImage(Kdo_Vulkan *vk, Kdo_VkImage *imageSrc1, Kdo_VkImage *imageSrc2, Kdo_mergeImageInfo info)
{
	...
	imageDst.memoryFilter	= imageSrc1->memoryFilter & imageSrc2->memoryFilter;
	imageDst.layout			= info.layout;
	...
	kdo_allocImage(vk, &imageDst, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, info.imageInfo);
	...
	for (j = 0; j < imageSrc1->divCount; j++)
	{
        ...
        imageBarrierInfo[i].srcAccessMask                   = 0;
        imageBarrierInfo[i].dstAccessMask                   = VK_ACCESS_TRANSFER_WRITE_BIT;
        imageBarrierInfo[i].oldLayout                       = imageSrc1->layout;
	...
	for (j = 0; j < imageSrc2->divCount; j++)
	{
        ...
        imageBarrierInfo[i].srcAccessMask                   = 0;
        imageBarrierInfo[i].dstAccessMask                   = VK_ACCESS_TRANSFER_WRITE_BIT;
        imageBarrierInfo[i].oldLayout                       = imageSrc2->layout;
        imageBarrierInfo[i].newLayout                       = VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL;
        imageBarrierInfo[i].srcQueueFamilyIndex             = VK_QUEUE_FAMILY_IGNORED;
        imageBarrierInfo[i].dstQueueFamilyIndex             = VK_QUEUE_FAMILY_IGNORED;
	...
	for (i = 0; i < imageSrc1->divCount; i++)
    {
		...
		imageBarrierInfo[i].newLayout       = imageDst.layout;
...
Kdo_VkImage	kdo_mergeBufferToImage(Kdo_Vulkan *vk, Kdo_VkBuffer *bufferSrc, Kdo_VkImage *imageSrc, Kdo_mergeBufferToImageInfo info)
{
	...
	imageDst.memoryFilter	= imageSrc->memoryFilter;
	imageDst.layout			= info.layout;
	...
	for (j = 0; j < imageSrc->divCount; j++)
	{
        ...
        imageBarrierInfo[i].srcAccessMask                   = 0;
        imageBarrierInfo[i].dstAccessMask                   = VK_ACCESS_TRANSFER_WRITE_BIT;
        imageBarrierInfo[i].oldLayout                       = imageSrc->layout;
        imageBarrierInfo[i].newLayout                       = VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL;
        imageBarrierInfo[i].srcQueueFamilyIndex             = VK_QUEUE_FAMILY_IGNORED;
        imageBarrierInfo[i].dstQueueFamilyIndex             = VK_QUEUE_FAMILY_IGNORED;
...
Kdo_VkBuffer	kdo_loadData(Kdo_Vulkan *vk, uint32_t infoCount, Kdo_VkLoadDataInfo *info)
{
	...
	bufferDst.usage		= VK_BUFFER_USAGE_TRANSFER_SRC_BIT; 
	bufferDst.size		= 0;
...
void	kdo_pushTextures(Kdo_Vulkan *vk, Kdo_VkImage *textures, uint32_t texturesCount, char **texturesPath)
{
	...
	Kdo_mergeBufferToImageInfo	mergeInfo;
	int							texWidth;
	int							texHeight;
	int							texChannels;
	uint32_t					i;
	...
	mergeInfo.imageInfo		= kdo_imageTextureInfo;
